...
/**
 * Add govph to body classes.
 *
 * @param array $classes array of body classes.
 * @return array array of body classes with 'govph' appended.
 */
function govph_body_class( $classes ) {
	$classes[] = 'govph';
	return $classes;
}
add_filter( 'body_class', 'govph_body_class', 10, 1 );
 ...
 */
function govph_filter_widget_display_callback( $settings, $widget, $args ) {
...
add_filter( 'widget_display_callback', 'govph_filter_widget_display_callback', 10, 3 );
 ...
 */
function govph_enter_title_here( $text, $post ) {
...
add_filter( 'enter_title_here', 'govph_enter_title_here', 10, 2 );
 ...
 */
function govph_print_menu_shortcode( $atts, $content = null ) {
...
add_shortcode( 'menu', 'govph_print_menu_shortcode' );
 ...
 * @return void
 */
function govph_register_sidebars() {
	register_sidebar(
		array(
			'id'          => 'front-page-before-loop',
			'name'        => __( 'Front Page Before Loop' ),
			'description' => __( 'Front page widget area before loop, first page only.', 'govph' ),
			'before_widget' => '',
			'after_widget' => '',
		)
	);
...
add_action( 'widgets_init', 'govph_register_sidebars' );
